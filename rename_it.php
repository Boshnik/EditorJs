...
function rename_extra($start_path, $find = array(), $replace = array())
{
    ...
    foreach ($items as $item) {
        ...
        foreach($find as $idx => $f) {
            if (strpos($old_path, $f) !== false) {
                $new_path = str_replace(
                    DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR,
                    DIRECTORY_SEPARATOR,
                    $start_path . DIRECTORY_SEPARATOR . str_replace($f, $replace[$idx], $item)
                );
        ...
        if (is_dir($new_path)) {
            ...
            if ($item == 'home.class.php') {
                $content = str_replace($replace[0] . 'ManagerController', 'EditorJsManagerController', $content);
            }
